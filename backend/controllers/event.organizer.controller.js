...
const getOrganizerEvents = async (req, res) => {
    try {
        ...
        return res.status(200).json({
            events: events.map((event) => formatOrgEvent(event))
        });
    } catch (error) {
...
const getOrganizerDashboardSummary = async (req, res) => {
    try {
        const events = await Event.find({ org_id: req.user.id }).select(
            'event_name reg_fee status event_start event_end'
        );

        const statsByEvent = await Promise.all(
            events.map(async (event) => {
                const stats = await getEventStats(event._id);
                return {
                    ...formatOrgEvent(event),
                    regCount: stats.registrationsCount,
                    attendance: stats.attendanceCount
                };
            })
        );

        const analytics = statsByEvent.reduce(
            (acc, event) => {
            ...
            { totalRegistrations: 0, totalRevenue: 0, totalAttendance: 0 }
        );
...
const publishEvent = async (req, res) => {
    try {
        ...
        if (
            ...
            (!event.event_registration_form || event.event_registration_form.length === 0)
        ) {
            return res
                .status(400)
                .json({ error: 'Define registration form fields before publishing a normal event' });
        }
        ...
        return res
            .status(200)
            .json({ message: 'Event published successfully', event: formatOrgEvent(event) });
    } catch (error) {
        return res.status(400).json({ error: error.message });
    }
};
...
const updateEventByOrganizer = async (req, res) => {
    try {
        ...
        if (effectiveStatus === 'DRAFT') {
            ...
            if (Object.prototype.hasOwnProperty.call(updates, 'event_description')) {
                event.event_description = updates.event_description;
            }
            ...
            if (Object.prototype.hasOwnProperty.call(updates, 'reg_deadline')) {
                ...
                if (newDeadline <= new Date(event.reg_deadline)) {
                    return res
                        .status(400)
                        .json({ error: 'Can only extend registration deadline for published events' });
                }
            ...
            if (Object.prototype.hasOwnProperty.call(updates, 'reg_limit')) {
                ...
                if (newLimit < Number(event.reg_limit)) {
                    return res
                        .status(400)
                        .json({ error: 'Can only increase registration limit for published events' });
                }
                event.reg_limit = newLimit;
            }

            if (Object.prototype.hasOwnProperty.call(updates, 'registration_closed')) {
                if (updates.registration_closed !== true) {
                    return res
                        .status(400)
                        .json({ error: 'Published events can only be closed for registrations' });
                }
            ...
            if (!Object.prototype.hasOwnProperty.call(updates, 'status')) {
                return res
                    .status(400)
                    .json({ error: 'Only status change is allowed for ongoing/completed events' });
            }

            const nextStatus = updates.status;
            if (!['COMPLETED', 'CLOSED'].includes(nextStatus)) {
                return res.status(400).json({ error: 'Allowed status transitions: COMPLETED or CLOSED' });
            }
        ...
        return res
            .status(200)
            .json({ message: 'Event updated successfully', event: formatOrgEvent(event) });
    } catch (error) {
        return res.status(400).json({ error: error.message });
    }
};
